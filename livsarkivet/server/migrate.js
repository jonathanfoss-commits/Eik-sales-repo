...
// Kjøres som databaseeier (ikke livsarkiv_app): node server/migrate.js
...
await klient.query('SELECT pg_advisory_lock(824301)');
...
// Managed Postgres krever passord på app-rollene. Settes de som miljø-
// variabler her, slipper man det manuelle ALTER ROLE-steget.
for (const [rolle, passord] of [
  ['livsarkiv_app', process.env.LIVSARKIV_APP_PASSORD],
  ['livsarkiv_auth', process.env.LIVSARKIV_AUTH_PASSORD],
]) {
  if (!passord) continue;
...
await klient.query('SELECT pg_advisory_unlock(824301)');
await klient.end();
