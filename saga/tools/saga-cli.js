 ...
 *  1. Leser saga/docs/improvements.md, prioriterer (effekt ↓, innsats ↑)
...
function improve() {
  ...
  if (!hasClaude) {
    ...
    sh(`git add saga/docs/changelog-auto.md && git commit -m "chore(saga): improve-plan (uten implementering – claude CLI mangler)"`);
  ...
  const prompt = `Implementer denne forbedringen i SAGA-repoet, minimal og testet: "${top.title}". ` +
    `Se saga/docs/improvements.md for kontekst. GUARDRAILS: rør aldri hemmeligheter/nøkler, ` +
...
else { console.log("Bruk: saga improve   (se saga/docs/automation.md)"); process.exit(cmd ? 1 : 0); }
